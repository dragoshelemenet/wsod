...
export default function OwnerFolderCard({
  ...
  const cleanPreviewImages = useMemo(() => {
    return [...new Set(previewImages.filter(Boolean))].filter(
      (src) => !failed.includes(src)
    );
  }, [previewImages, failed]);

  const mainImage =
    imageUrl && !failed.includes(imageUrl)
      ? imageUrl
      : cleanPreviewImages[0] ?? null;
  ...
  return (
    ...
    >
      ...
      <div className="folder-body owner-folder-classic-body">
        {mainImage ? (
          <div
            className={`folder-brand-art-wrap${
              useTransparentArt ? " folder-brand-art-wrap-transparent" : ""
            }`}
            aria-hidden="true"
          >
            <MainMedia
              ...
              className={`folder-brand-art${
                useTransparentArt ? " folder-brand-art-transparent" : ""
              }`}
            ...
            {hoverImages[0] ? (
              <div className="folder-hover-shot folder-hover-shot-1">
                <HoverMedia
                  ...
                  onError={() =>
                    setFailed((current) => [...current, hoverImages[0]])
                  }
                />
              </div>
            ) : null}
            ...
            {hoverImages[1] ? (
              <div className="folder-hover-shot folder-hover-shot-2">
                <HoverMedia
                  ...
                  className="folder-hover-media"
                  onError={() =>
                    setFailed((current) => [...current, hoverImages[1]])
                  }
                />
              </div>
            ) : null}
            ...
            {hoverImages[2] ? (
              <div className="folder-hover-shot folder-hover-shot-3">
                <HoverMedia
                  ...
                  className="folder-hover-media"
                  onError={() =>
                    setFailed((current) => [...current, hoverImages[2]])
                  }
                />
              </div>
            ) : null}
