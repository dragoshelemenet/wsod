...
export async function generateMetadata({ params }: VideoDetailPageProps): Promise<Metadata> {
  ...
  if (!item || item.category !== "video") return { title: "Video | WSOD.PROD" };
  const title = item.seoTitle || `${item.title} | Video | WSOD.PROD`;
  const description = item.metaDescription || item.description || `Lucrare video din portofoliul WSOD.PROD: ${item.title}.`;
...
function getOwnerAllLabel(item: NonNullable<Awaited<ReturnType<typeof getMediaItemBySlugFromDb>>>) {
  if (item.owner.type === "model") return `Vezi toate clipurile cu ${item.owner.name}`;
  if (item.owner.type === "brand") return `Vezi toate clipurile pentru ${item.owner.name}`;
  if (item.owner.type === "audioProfile") return `Vezi toate clipurile pentru ${item.owner.name}`;
...
export default async function VideoDetailPage({ params }: VideoDetailPageProps) {
  ...
  const [sameOwnerItems, randomItems] = await Promise.all([
    ...
    getRandomMediaByCategoryFromDb("video", 6, item.owner.type !== "unknown" ? { type: item.owner.type, slug: item.owner.slug } : undefined),
  ]);
  ...
  return (
    <main className="inner-page">
      ...
      <div className="inner-topbar"><Link href="/video" className="back-link">← Înapoi la video</Link></div>

      <section className="inner-section">
        <MediaBreadcrumbs categoryLabel="Video" categoryHref="/video" ownerName={item.owner.name} ownerHref={ownerHref} currentTitle={item.title} />
        <h1>{item.title}</h1>
        <p className="inner-description">{item.description || "Lucrare video din portofoliul WSOD.PROD."}</p>

        {previewVisual ? <div className="media-detail-hero"><img src={previewVisual} alt={item.title} className="media-detail-image" /></div> : null}

        <div className="media-detail-meta">
          <p><strong>Categorie:</strong> Video</p>
        ...
        <div className="media-actions">
          ...
          {item.fileUrl ? <a href={item.fileUrl} target="_blank" rel="noreferrer" className="media-open-button">Deschide video</a> : null}
        </div>

        <div className="owner-folder-section">
          <div className="owner-folder-section-head"><h2>{`Mai multe video pentru ${item.owner.name}`}</h2></div>
          <MediaGrid items={sameOwnerItems} emptyText="Nu există alte video similare momentan." />
          <div className="model-page-actions">{ownerHref ? <Link href={ownerHref} className="media-open-button">{getOwnerAllLabel(item)}</Link> : null}</div>
        </div>

        <div className="owner-folder-section">
          <div className="owner-folder-section-head"><h2>Alte video</h2></div>
          <MediaGrid items={randomItems} emptyText="Nu există alte video momentan." />
          <div className="model-page-actions"><Link href="/video" className="media-link">Vezi toate video</Link></div>
        </div>
      </section>
    </main>
  );
